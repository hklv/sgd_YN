...
(function( $ ) {
  $.fn.promptumenu = function(options) {
	...
	var settings = $.extend({
		...
		'pages': true,
		'inertia': 200,
		'windowCss':'promptumenu_window',
		'navCss':'promptumenu_nav'
	}, options);
	...
	return this.each(function(){
		...
		var methods = {
			...
			go_to: function(index, easing, webkit){
				...
				$this.parent('.'+settings.windowCss).find('.'+settings.navCss+' a.active').removeClass('active');
				$this.parent('.'+settings.windowCss).find('.'+settings.navCss+' a:nth-child(' + (index) + ')').addClass('active');
		...
		if($this.data('promptumenu')){
			...
			$this.wrap('<div class="'+settings.windowCss+'" />');
			$this.parent('.'+settings.windowCss).css(properties);
			$this.css({
				'display': 'block',
				'position': 'absolute',
				'list-style': 'none',
			...
			if(cells.pages > 1 && settings.pages == true){
				...
				$this.parent('div.'+settings.windowCss).append('<div class="'+settings.navCss+'">' + page_links + '</div>');
				
				//bind the nav buttons to navigate to the specific page
				$this.parent('div.'+settings.windowCss).find('.'+settings.navCss+' a').bind('click.promptumenu', function(){
