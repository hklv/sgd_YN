...
(function(){
    ...
    function initVideo(){
        ...
        (function(){
            ...
            $G("videoWidth").value = initWidth;
            $G("videoHeight").value = initHeight;
...
//        }
        // 匹配自家上传的视频
        var myVideo = url.match(/(\/upload\/\d{4}\/\d{2}\/\d{2}\/\d{13})\.[^\/]+/);
        if(myVideo){
        	$G("preview").innerHTML = '<div id="myVideo"></div>';
			jwplayer("myVideo").setup({
				autostart : false,
				file : editor.options.videoPath + myVideo[0],
				image : "",//editor.options.videoPath + myVideo[1] +".png",
				width : 420,
				height : 280
			});
        	return;
        ...
        var youku = url.match(/youku\.com\/v_show\/id_(\w+)/),
	        youkuPlay = /player\.youku\.com/ig.test(url);
        var tudou = url.match(/tudou\.com\/programs\/view\/(\w+)/),
        	tudouPlay = /www\.tudou\.com\/v/ig.test(url);
	    if(!youkuPlay){
	    	if(!tudouPlay){
		        if(youku){
		            url = "http://player.youku.com/player.php/sid/"+youku[1]+"/v.swf";
		        }else if(tudou){
		            url = "http://www.tudou.com/v/"+tudou[1];
		        }else if(!endWith(url,[".swf",".flv",".wmv"])){
		            $G("preview").innerHTML = lang.urlError;
		            return;
		        }
        ...
        ' width="420"' ,
        ' height="280"' ,
        ' wmode="transparent" play="true" loop="false" menu="false" allowscriptaccess="never" allowfullscreen="true" ></embed>'].join('');
    }
    ...
    function insertUpload(){
        var videoObjs=[],
            ...
            width = $G('upload_width').value || initWidth,
            height = $G('upload_height').value || initHeight,
...
//                status.innerHTML = ((num && num[0] ?parseInt(num[0]):0) + numFilesUploaded) +lang.statusPrompt;
        ...
        $G('upload_width').value || ($G('upload_width').value = initWidth);
        $G('upload_height').value || ($G('upload_height').value = initHeight);
    };
