...
var LineTool = com.xjwgraph.LineTool = function(d) {
	...
	if (c.isSVG) {
		...
		c.svgBody.appendChild(diamond);             
		c.svgBody.appendChild(slash);    
		...
		createBaseLine : function(b, h, a) {   //b=lineId h=path a=brokenType
			...
			if (e.isSVG) {
				...
				if (e.isVML) {
					...
					d.appendChild(f);					
					var title = g.createElement('<v:rect style="POSITION: absolute;font-size:12;top:10px;word-break:break-all;width:30px;"></v:rect>');	
