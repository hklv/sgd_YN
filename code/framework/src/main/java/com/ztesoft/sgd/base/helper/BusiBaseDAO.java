...
public class BusiBaseDAO extends BaseDAO {
    ...
    public List<Object> queryListToSimpObj(String querySql, ParamArray queryParam, final Class<?> objClass)
        ...
        return query(querySql, queryParam, null, new RowSetMapper<List<Object>>() {
            public List<Object> mapRows(RowSetOperator op, ResultSet rs, int colNum, Object para) throws SQLException,
                    BaseAppException {
                return mapRowsForList(rs, objClass);
            }
        });
    }
    ...
    protected  <T> List<T> mapRowsForList(ResultSet rs, final Class<?> objClass) throws SQLException, BaseAppException {
        ...
        try {
            while (rs.next()) {
                objList.add((T) toSimpleObj(resultSetMetaData, rs, columnCount, objClass));
            }
        } catch (Exception e) {
    ...
    public List<Object> queryListToSimpObj2(String querySql, ParamArray queryParam, final Class<?> objClass,
        ...
        return query(querySql, queryParam, null, new RowSetMapper<List<Object>>() {
            public List<Object> mapRows(RowSetOperator op, ResultSet rs, int colNum, Object para) throws SQLException,
                    BaseAppException {
                return mapRowsForListObject(rs, primaryKey, objClass, childObjClass, childFieldName);
            }
        });
    }
    ...
    private List<Object> mapRowsForListObject(ResultSet rs, String primaryKey, Class<?> objClass, Class<?> childObjClass, String childFieldName) throws SQLException, BaseAppException {
    ...
    public Object queryToSimpObj2(String querySql, ParamArray queryParam, final Class<?> objClass,
        ...
        return query(querySql, queryParam, null, new RowSetMapper<Object>() {
            public Object mapRows(RowSetOperator op, ResultSet rs, int colNum, Object para) throws SQLException,
                    BaseAppException {
                return mapRowsForObj(rs, objClass, childObjClass, childFieldName);
            }
        });
    }
    ...
    private Object mapRowsForObj(ResultSet rs, Class<?> objClass, Class<?> childObjClass, String childFieldName) throws SQLException, BaseAppException {
        ResultSetMetaData rsmd = rs.getMetaData();
        int columnCount = rsmd.getColumnCount();
        Object tempObj = null;
        List<Object> childTempList = null;
    ...
    public <T> T qryToArrayObj(String qrySql, ParamArray qryParam, final Class<?> objClass) throws BaseAppException {
        return query(qrySql, qryParam, null, new RowSetMapper<T>() {
            public T mapRows(RowSetOperator op, ResultSet rs, int colNum, Object para) throws SQLException,
                    BaseAppException {
                return mapRowForObj(rs, objClass);
            }
        });
    }
    ...
    private <T> T mapRowForObj(ResultSet rs, Class<?> objClass) throws SQLException, BaseAppException {
        ResultSetMetaData rsmd = rs.getMetaData();
        int columnCount = rsmd.getColumnCount();
    ...
    public Object qryToArrayObj2(String querySql, ParamArray queryParam, final Class<?> objClass,
        ...
        return query(querySql, queryParam, null, new RowSetMapper<Object>() {
            public Object mapRows(RowSetOperator op, ResultSet rs, int colNum, Object para) throws SQLException,
                    BaseAppException {
                return qryToArrayObj(rs, primaryKey, objClass, childObjClass, childFieldName);
            }
        });
    }
    ...
    private Object qryToArrayObj(ResultSet rs, String primaryKey, Class<?> objClass, Class<?> childObjClass, String childFieldName) throws SQLException, BaseAppException {
        ResultSetMetaData rsmd = rs.getMetaData();
        int columnCount = rsmd.getColumnCount();
        ...
        if (BoHelper.isSimpleType(retObj)) {
            ...
            for (int i = 1; i <= columnCount; i++) {
                ...
                try {
                    ...
                    // 不理会带$的字段
                    if (columnName.contains(IGNORE_FIELD_SIGN)) {
                        continue;
                    }
     ...
     * @taskId <br>
     */
    public ParamMap getParamMap(Object obj) throws BaseAppException {
     ...
     * @taskId <br>
     */
    public void setParamMap(ParamMap paramMap, String paramName, Object paramValue, Class<?> fileType) throws BaseAppException {
        JdbcUtil4SGD.setParamMap(paramMap, paramName, paramValue, fileType);
    }
     ...
     * @taskId <br>
     */
    public ParamArray getParamArray(Object... args) throws BaseAppException {
    ...
    public Object toSimpleObj2(ResultSetMetaData rsmd, ResultSet rs, int columnCount, Class<?> retObj,
        ...
        for (int i = 1; i <= columnCount; i++) {
            ...
            makeColumn(columnName, methodNameBuff, fieldNameBuff);
    ...
    private void callSurperMethod3(String fieldName, Class<?> superClass, String methodName, Object obj,
        ...
        try {
            ...
            superClass = superClass.getSuperclass();
            if (superClass.equals(Object.class) || superClass.getDeclaredFields().length == 0) {
                ExceptionHandler.publish("callSuperMethod Has Exception", e);
            }

            callSurperMethod3(fieldName, superClass, methodName, obj, parentObj);
        } catch (Exception e) {
            ExceptionHandler.publish("callSuperMethod3 Has Exception", e);
        }
    }
    ...
    private void makeColumn(String columnName, StringBuffer methodNameBuff, StringBuffer fieldNameBuff) {
    ...
    public DynamicDict selectObject(String selectSql, Object args) throws BaseAppException {
        List list;
        if (BoHelper.isSimpleType(args.getClass())) {
            list = this.queryListToDict(selectSql, this.getParamArray(args));
        } else {
            list = this.queryListToDict(selectSql, this.getParamMap(args), null);
        }

        return list != null && list.size() > 0 ? (DynamicDict) list.get(0) : null;
    }

    private List<DynamicDict> queryListToDict(String querySql, ParamArray queryParam) throws BaseAppException {
        return this.query(querySql, queryParam, null, new RowSetMapper<List<DynamicDict>>() {
            public List<DynamicDict> mapRows(RowSetOperator op, ResultSet rs, int colNum, Object para) throws SQLException, BaseAppException {
                return mapRowsForDynamicDict(op, rs);
            }
        });
    }

    private List<DynamicDict> queryListToDict(String querySql, ParamMap queryParam, RowSetFormatter rowSetFormatter) throws BaseAppException {
        return this.query(querySql, queryParam, rowSetFormatter, null, new RowSetMapper<List<DynamicDict>>() {
            public List<DynamicDict> mapRows(RowSetOperator op, ResultSet rs, int colNum, Object para) throws SQLException, BaseAppException {
                return mapRowsForDynamicDict(op, rs);
            }
        });
    ...
    private List<DynamicDict> mapRowsForDynamicDict(RowSetOperator op, ResultSet rs) throws BaseAppException {
        List<DynamicDict> objList = new ArrayList<DynamicDict>();
        String[] colNames = op.getColNames();
        try {
            while (rs.next()) {
                DynamicDict e = new DynamicDict();
                for (String colName : colNames) {
                    e.set(colName, op.getValue(rs, colName));
                }
                objList.add(e);
            }
        } catch (Exception var9) {
            ExceptionHandler.publish("queryListToSimpObj has exception:", var9);
        }
        return objList;
    }

    public void recuriveList(Map<String, String> keyMap, List<DynamicDict> list, List<DynamicDict> newList) throws BaseAppException {

        for (DynamicDict dict : list) {
            if ((keyMap.get("PARENT_CODE")).equals(dict.getString(keyMap.get("PARENT_ID")))) {
                newList.add(dict);
                Map<String, String> tmpMap = new HashMap<String, String>();
                tmpMap.putAll(keyMap);
                tmpMap.put("PARENT_CODE", dict.getString(keyMap.get("ID")));
                this.recuriveList(tmpMap, list, newList);
            }
        }

    }

    public long selectCount(String selectSql, Object args) throws BaseAppException {
        Object obj;
        if (BoHelper.isSimpleType(args.getClass())) {
            obj = this.queryToSimpObj(selectSql, this.getParamArray(new Object[]{args}), Long.class);
            return obj == null ? 0L : (Long) obj;
        } else {
            obj = this.queryToSimpObj(selectSql, this.getParamMap(args), Long.class);
            return obj == null ? 0L : (Long) obj;
        }
    }
}
