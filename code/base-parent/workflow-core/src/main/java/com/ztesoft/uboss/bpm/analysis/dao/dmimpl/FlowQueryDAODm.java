...
public class FlowQueryDAODm extends FlowQueryDAO {
	...
	public List<DelayedFlows> selectDelayedFlows(
		...
		String sql = "SELECT COUNT(1) DELAYED_FLOW_NUMBER, TRUNC(A.START_TIME, 'month') ANALYSE_MONTH"
			...
			+ " AND B.PROC_TEMP_ID = C.PROC_TEMP_ID AND (((ADD_DAYS(A.START_TIME, B.OVER_TIME)) > NOW() AND A.HOLDER_STATE NOT IN ('C', 'T'))"
			+ " OR ((ADD_DAYS(A.START_TIME, B.OVER_TIME)) > A.END_TIME AND A.HOLDER_STATE = 'C'))"
