...
public class Parser {
	 ...
	 * @return the consumed token (which was the current token when calling this method)
	 */
	protected final Token consumeInToken() throws ScanException, ParseException {
		...
		while (true) {
			switch (token.getSymbol()) {
				...
				case IN:
					//fist consume '('
					consumeToken();
					//then consume String context
					v = createAstBinary(v, new AstString(consumeInToken().getImage()), AstBinary.IN);
					//last consume ')'
					consumeToken();
					consumeToken(RPAREN);
					break;
				case NOTIN:
					//fist consume '('
					consumeToken();
					v = createAstBinary(v, new AstString(consumeInToken().getImage()), AstBinary.NOTIN);
					//last consume ')'
					consumeToken();
					consumeToken(RPAREN);
					break;
