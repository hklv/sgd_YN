...
public class BpmnActivityBehavior {
  ...
  protected void performOutgoingBehavior(ActivityExecution execution, 
    ...
    //for jump flow
    Object obj = execution.getVariableLocal("isjump");
    if("y".equals(obj))
    {
    	List<PvmTransition> dynOutgoingTransitions = execution.getActivity().getDynOutgoingTransitions();
    	
    	for (PvmTransition outgoingTransition : dynOutgoingTransitions) {
    		transitionsToTake.add(outgoingTransition);
    	}
    	
    	execution.removeVariableLocal("isjump");
    	dynOutgoingTransitions.clear();
    }
    else
    {
	    for (PvmTransition outgoingTransition : outgoingTransitions) {
	      if (defaultSequenceFlow == null || !outgoingTransition.getId().equals(defaultSequenceFlow)) {
	        ...
	        //add service condition support
	        if(condition == null)
	        {
	        	condition = (Condition) outgoingTransition.getProperty(BpmnParse.PROPERTYNAME_SERVICE_CONDITION);
	        }
