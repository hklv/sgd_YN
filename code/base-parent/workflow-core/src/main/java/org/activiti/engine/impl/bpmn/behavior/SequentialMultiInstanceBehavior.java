...
import org.activiti.uboss.behavior.ZMultiInstanceActivityBehavior;
 ...
 * @author Falko Menge
 */
public class SequentialMultiInstanceBehavior extends ZMultiInstanceActivityBehavior {
...
//  }
	...
	public SequentialMultiInstanceBehavior(ActivityImpl activity,
		...
		super(activity, innerActivityBehavior,taskTemplateId);
	}
  ...
  protected void createInstances(ActivityExecution execution) throws Exception {
	...
	int index = 0;
	Object obj = execution.getVariableLocal("_designate_user_s");
	if (obj == null) {
		execution.setVariableLocal("_designate_user_s", Long.valueOf(0));
	} else {
		index = ((Long) obj).intValue();
		index++;
	}
	execution.setVariableLocal("_designate_assgin", getUsers().get(index));
