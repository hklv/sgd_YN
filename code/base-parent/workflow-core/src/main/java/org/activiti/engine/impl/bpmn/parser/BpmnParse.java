...
import org.activiti.uboss.behavior.ZBusinessRuleTaskActivityBehavior;
import org.activiti.uboss.behavior.ZMailActivityBehavior;
import org.activiti.uboss.behavior.ZNoneEndEventActivityBehavior;
import org.activiti.uboss.behavior.ZNoneStartEventActivityBehavior;
import org.activiti.uboss.behavior.ZScriptTaskActivityBehavior;
import org.activiti.uboss.behavior.ZSubProcessNoneStartEventActivityBehavior;
import org.activiti.uboss.behavior.ZUserTaskActivityBehavior;
import org.activiti.uboss.el.ZServiceCondition;
...
import org.activiti.uboss.parser.ZTaskListenerImpl;
...
public class BpmnParse extends Parse {
	...
	public static final String PROPERTYNAME_SERVICE_CONDITION = "serviceCondition";
	 ...
	 * create instances. liyb modify to public
	 */
	...
	protected void parseProcessDefinitionStartEvent(
		...
		startEventActivity
				.setActivityBehavior(new ZNoneStartEventActivityBehavior());
...
//							.setActivityBehavior(new ZNoneStartEventActivityBehavior());
					startEventActivity
					.setActivityBehavior(new ZSubProcessNoneStartEventActivityBehavior());
				} else {
					addError(
...
//					signalDefinition);
	...
	public void parseMultiInstanceLoopCharacteristics(Element activityElement,
		...
		String taskTemplateId = activityElement.attributeNS(
				BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS, "taskTemplateId");
			...
			if (isSequential) {
				miActivityBehavior = new SequentialMultiInstanceBehavior(
						...
						(AbstractBpmnActivityBehavior) activity
								.getActivityBehavior(), taskTemplateId);
			} else {
...
//			}
	...
	public ActivityImpl parseScriptTask(Element scriptTaskElement,
		...
		activity.setActivityBehavior(new ZScriptTaskActivityBehavior(script,
	...
	public ActivityImpl parseServiceTask(Element serviceTaskElement,
		...
		String resultVariableName = serviceTaskElement.attributeNS(
				BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS, "resultVariable");
		String taskTemplateId = serviceTaskElement.attributeNS(
				BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS, "taskTemplateId");
		
		String blockFlag = serviceTaskElement.attributeNS(
				BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS, "blockFlag");
		...
		// add by liyb
		else if (taskTemplateId != null && taskTemplateId.trim().length() > 0) {
			
			activity.setProperty("taskTemplateId", taskTemplateId);
			
			/** TODO 需要传递taskTemplateId到 对应的behavior中 */
			Element extentionsElement = __getExtentionsElement__(serviceTaskElement);
			ZServiceDefinition serviceDefinition = __newServiceDefinition__(
					taskTemplateId, resultVariableName, extentionsElement);
			...
			// 通过反射把当前 任务信息 送至delegate类中，如:ServiceDelegate
			List<FieldDeclaration> fieldDeclarations = new ArrayList<FieldDeclaration>();
			FieldDeclaration fieldDeclaration = new FieldDeclaration(
					"callableDefinition", ZServiceDefinition.class.getName(),
					serviceDefinition);
			fieldDeclarations.add(fieldDeclaration);

			ZClassDelegate zDelegate = new ZClassDelegate(
					"com.ztesoft.uboss.bpm.runtime.task.ServiceDelegate",
					fieldDeclarations);
			zDelegate.setBlockFlag(blockFlag);
			activity.setActivityBehavior(zDelegate);
			...
			//LOGGER.info("serviceTask info is not completed");
			//addError("serviceTask info is not completed", serviceTaskElement);
		}
	...
	public ActivityImpl parseBusinessRuleTask(Element businessRuleTaskElement,
		...
		BusinessRuleTaskActivityBehavior ruleActivity = new ZBusinessRuleTaskActivityBehavior();
	...
	protected void parseMuleServiceTask(ActivityImpl activity,
		...
		activity.setActivityBehavior((ActivityBehavior) ClassDelegate
				.instantiateDelegate(
						org.activiti.uboss.behavior.ZMQSendActivitiBehavior.class,
						fieldDeclarations));
...
//						fieldDeclarations));
		...
		activity.setActivityBehavior((ZMailActivityBehavior) ClassDelegate
				.instantiateDelegate(ZMailActivityBehavior.class,
						fieldDeclarations));
	}
	...
	public ActivityImpl parseUserTask(Element userTaskElement, ScopeImpl scope) {
		...
		String taskTemplateId = userTaskElement.attributeNS(
				BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS, "taskTemplateId");
		
		activity.setProperty("taskTemplateId", taskTemplateId);

//		if(taskTemplateId == null || "".equals(taskTemplateId))
//		{
//			addError("userTask info is not completed, taskTemplateId is empty", userTaskElement);
//		}
		
		UserTaskActivityBehavior userTaskActivity = new ZUserTaskActivityBehavior(
				taskTemplateId, expressionManager, taskDefinition);
	...
	protected void parseUserTaskCustomExtensions(Element taskElement,
		...
		parseUbossTaskListeners(taskElement, taskDefinition);
	...
	protected void parseUbossTaskListeners(Element userTaskElement,
			TaskDefinition taskDefinition) {
		String taskTemplateId = userTaskElement.attributeNS(
				BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS, "taskTemplateId");
		...
		if(StringUtil.isNotEmpty(taskTemplateId)){
			...
			List<BpmTaskEventDto> events = ZExtendsionUtil.qryTaskTemplateEvent(taskTemplateId);
			
			for(BpmTaskEventDto ubossEvent : events){
				
				TaskListener taskListener = new ZTaskListenerImpl(ubossEvent);
				
				ZServiceDefinition serviceDefinition = __newServiceDefinition__(
						taskTemplateId, null, userTaskElement);
				
				((ZTaskListenerImpl)taskListener).setServiceDefinition(serviceDefinition);
				
				if("START".equals(ubossEvent.getEventType())){

					taskDefinition.addTaskListener(TaskListener.EVENTNAME_CREATE, taskListener);	
				}else if("END".equals(ubossEvent.getEventType())){

					taskDefinition.addTaskListener(TaskListener.EVENTNAME_COMPLETE, taskListener);	
				}else if("BACK".equals(ubossEvent.getEventType())){

					taskDefinition.addTaskListener(TaskListener.EVENTNAME_BACK, taskListener);	
				}
			}
	...
	public void parseEndEvents(Element parentElement, ScopeImpl scope) {
		for (Element endEventElement : parentElement.elements("endEvent")) {
			...
			if (errorEventDefinition != null) { // error end event
				...
				activity.setActivityBehavior(new ZNoneEndEventActivityBehavior());
			}
...
//	}
	...
	public void parseSequenceFlowConditionExpression(Element seqFlowElement,
		...
		Element serviceExprElement = seqFlowElement
		...
		//for service condition support
		else if(serviceExprElement != null)
		{
			String expression = serviceExprElement.getText().trim();
			
			if(StringUtil.isEmpty(expression) || expression.indexOf(".") == -1)
			{
				addError("Invalid serviceExpression text", serviceExprElement);
			}
			else
			{
				String[] expressions = expression.split("\\.");
				Condition serviceExpressionCondition = new ZServiceCondition(expressions[0], expressions[1]);
				seqFlow.setProperty(PROPERTYNAME_SERVICE_CONDITION, serviceExpressionCondition);
			}
		}
	}
	...
	private ZServiceDefinition __newServiceDefinition__(String templateId,
			String resultVariableName, Element extentionsElement) {
 
		ZServiceDefinition serviceDefinition = new ZServiceDefinition(
				templateId, resultVariableName, null, null);
		return serviceDefinition;
	}

	private Element __getExtentionsElement__(Element element) {
		Element extentionsElement = element
				.element(ZExtendsionUtil.BPMN_20_N_ExtensionElements);
		return extentionsElement;
	}
}
