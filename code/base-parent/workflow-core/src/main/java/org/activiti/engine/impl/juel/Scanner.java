...
public class Scanner {
	...
	public enum Symbol {
		...
		LPAREN("'('"), RPAREN("')'"),
		IDENTIFIER,
		NOT("'!'|'not'"), AND("'&&'|'and'"), OR("'||'|'or'"),IN("'in'"),NOTIN("'notin'"),
	...
	static {
		...
		addFixToken(new Token(Symbol.IN, "in"));
		addFixToken(new Token(Symbol.NOTIN, "notin"));
	...
	protected Token nextEval() throws ScanException {
		...
		switch (c1) {
			...
			case 'i': 
				if (c2 == 'n') {
					return fixed(Symbol.IN);
				}
				break;
			case 'n': 
				if (c2 == 'o') {
					return fixed(Symbol.NOTIN);
				}
				break;
		}
	...
	//for 'in' and 'notin' input
	public Token next2() throws ScanException {
		if (token != null) {
			position += token.getSize();
		}
		...
		int i = position+1;
		int l = input.length();
		while (i < l && input.charAt(i) != ')') {
			i++;
		}
		String name = input.substring(position, i).replace("\'", "");
		return  token = token(Symbol.IDENTIFIER, name, i - position);
	}
}
