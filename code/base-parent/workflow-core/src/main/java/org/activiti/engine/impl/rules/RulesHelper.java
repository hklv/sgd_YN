...
public class RulesHelper {
	...
	public static KnowledgeBase findKnowledgeBaseByDeploymentId(
		...
		KnowledgeBase knowledgeBase = (KnowledgeBase) knowledgeBaseCache
				.get(deploymentId);
		if (knowledgeBase == null) {
			DeploymentEntity deployment = Context.getCommandContext()
					.getDeploymentManager().findDeploymentById(deploymentId);
			if (deployment == null) {
				throw new ActivitiException("no deployment with id "
						+ deploymentId);
			}
			Context.getProcessEngineConfiguration().getDeploymentCache()
					.deploy(deployment);
			knowledgeBase = (KnowledgeBase) knowledgeBaseCache
					.get(deploymentId);
			// modify by liyb
			// if (knowledgeBase==null) {
			// throw new
			// ActivitiException("deployment "+deploymentId+" doesn't contain any rules");
			// }
		}
